...
The repo ships with three short **synthesized placeholder tracks** so the
reactive visuals work out of the box. Replace them by adding your own
`.mp3` / `.wav` / `.ogg` files to `public/media/` and pointing the `tracks`
entries at them. Audio is streamed (not preloaded), so large files are fine.

The placeholders are generated by a small script — regenerate them any time:

```bash
npm run generate:media
```

This also creates the SVG placeholder artwork and the six visual placeholders.
